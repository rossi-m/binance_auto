...
import json  # 导入json库，用于和 DeepSeek 交换结构化决策数据
import requests  # 导入requests库，用于调用 DeepSeek API
...
# --- DeepSeek AI 审核层配置 ---
DEEPSEEK_AI_ENABLED = read_bool_env('DEEPSEEK_AI_ENABLED', True)  # 新脚本默认启用 AI 审核
DEEPSEEK_API_KEY = os.getenv('DEEPSEEK_API_KEY', '').strip()  # DeepSeek API Key
DEEPSEEK_BASE_URL = os.getenv('DEEPSEEK_BASE_URL', 'https://api.deepseek.com').strip().rstrip('/')  # DeepSeek API Base URL
DEEPSEEK_MODEL = os.getenv('DEEPSEEK_MODEL', 'deepseek-v4-pro').strip() or 'deepseek-v4-pro'  # 默认使用用户指定的 pro 模型
DEEPSEEK_THINKING_ENABLED = read_bool_env('DEEPSEEK_THINKING_ENABLED', True)  # 默认显式开启思考模式
DEEPSEEK_TIMEOUT_SECONDS = float(os.getenv('DEEPSEEK_TIMEOUT_SECONDS', '18'))  # 单次 AI 审核超时
DEEPSEEK_MAX_TOKENS = int(os.getenv('DEEPSEEK_MAX_TOKENS', '700'))  # 限制 AI 输出长度，避免响应太臃肿
DEEPSEEK_BAR_LIMIT = max(2, int(os.getenv('DEEPSEEK_BAR_LIMIT', '3')))  # 只给 AI 最近少量K线，控制 token
DEEPSEEK_ALLOW_ENTRY_ON_ERROR = read_bool_env('DEEPSEEK_ALLOW_ENTRY_ON_ERROR', False)  # AI 出错时默认不新开仓
DEEPSEEK_ALLOW_EXIT_ON_ERROR = read_bool_env('DEEPSEEK_ALLOW_EXIT_ON_ERROR', True)  # AI 出错时保留原策略平仓能力
...
def to_plain_value(value):
    """把 pandas / datetime / numpy 风格对象压平成可 JSON 序列化的基础类型。"""
    if isinstance(value, dict):
...
def get_recent_bar_snapshot(df, timeframe, count=DEEPSEEK_BAR_LIMIT, now_dt=None):
    """提取最近几根已收盘K线及关键指标，给 AI 提供简洁行情上下文。"""
    if df is None or len(df) == 0:
        return []
...
def get_state_snapshot_for_ai(timeframe, state):
    """从状态字典中挑出适合给 AI 的关键字段，避免把整个 details 全量塞过去。"""
    if not isinstance(state, dict):
        return {'timeframe': timeframe, 'missing': True}

    keys = [
        'signal_bar_time', 'long_trend', 'short_trend', 'pullback_long', 'pullback_short',
        'close_long', 'close_short', 'is_oscillation', 'boll_band_width_ratio',
        'oscillation_threshold', 'upper_shadow_filter_long', 'lower_shadow_filter_short',
        'upper_shadow_reversal_short', 'lower_shadow_reversal_long',
        'upper_shadow_tighten_long', 'lower_shadow_tighten_short',
        'shadow_short_stop_ref', 'shadow_long_stop_ref',
        'upper_shadow_long_protect_ref', 'lower_shadow_short_protect_ref'
    ]
    snapshot = {'timeframe': timeframe}
    for key in keys:
        snapshot[key] = to_plain_value(state.get(key))
    snapshot['mail_snapshot'] = format_condition_snapshot_for_mail(timeframe, state)
    return snapshot


def get_position_snapshot_for_ai(curr_price=None):
    """整理当前持仓状态，让 AI 在平仓/反手审核时知道仓位背景。"""
    ...
    return {
        'has_position': True,
        'side': trade_state.get('side'),
        'entry_price': to_plain_value(trade_state.get('entry_price')),
        ...
        'highest_price': to_plain_value(trade_state.get('highest_price')),
        'lowest_price': to_plain_value(trade_state.get('lowest_price')),
        'current_price': to_plain_value(curr_price),
        'unrealized_points': None if curr_price is None else round(
            (curr_price - trade_state.get('entry_price', 0))
            if trade_state.get('side') == 'long'
            else (trade_state.get('entry_price', 0) - curr_price),
            6
        )
    }


def build_market_snapshot_for_ai(df_4h, df_1h, df_15m, state_4h, state_1h, state_15m, signal_bar_15m, server_now_dt=None):
    """构建给 AI 的紧凑市场上下文。"""
    ...
    return {
        ...
        'timeframes': {
            '4h': {
                'state': get_state_snapshot_for_ai('4H', state_4h),
                'bars': get_recent_bar_snapshot(df_4h, '4h', now_dt=server_now_dt)
            },
            '1h': {
                'state': get_state_snapshot_for_ai('1H', state_1h),
                'bars': get_recent_bar_snapshot(df_1h, '1h', now_dt=server_now_dt)
            },
            '15m': {
                'state': get_state_snapshot_for_ai('15M', state_15m),
...
def call_deepseek_json(system_prompt, user_payload):
    """调用 DeepSeek Chat Completions，并要求返回严格 JSON。"""
    if not DEEPSEEK_AI_ENABLED:
        return {
            'decision': 'allow',
            'confidence': 0.0,
            'reason': 'DeepSeek AI 已通过环境变量关闭，回退到原策略。'
        }
...
def format_ai_review_text(ai_review):
    ...
    risk_text = f", risk_flags={';'.join(str(item) for item in risk_flags[:3])}" if risk_flags else ''
    return f"AI decision={decision}{confidence_text}, reason={reason}{risk_text}".strip()


def review_entry_with_ai(candidate_side, entry_reason, trigger_tf, curr_price, stop_loss_price, market_snapshot):
    """让 DeepSeek 对策略给出的候选开仓做二次审核。"""
    context = {
        'task': 'entry_review',
        'candidate_side': candidate_side,
        'entry_reason': entry_reason,
        'trigger_timeframe': trigger_tf or '4H+1H+15M',
        'current_price': curr_price,
        'proposed_stop_loss': stop_loss_price,
        'market': market_snapshot
    }
    system_prompt = (
        '你是 ETH/USDT 永续合约实盘系统的 AI 风控审核层。'
        '基础量化策略已经给出候选开仓，你只能在“allow”或“reject”之间选择。'
        '优先考虑多周期一致性、是否震荡、是否出现反向影线/平仓信号、止损空间是否合理。'
        '必须只返回 JSON 对象，字段固定为 decision, confidence, reason, risk_flags。'
    )
...
def review_exit_with_ai(position_side, proposed_action, trigger_label, curr_price, market_snapshot):
    """让 DeepSeek 决定是否接受原策略的主动平仓/反手信号。"""
    context = {
        'task': 'exit_review',
    ...
    system_prompt = (
        '你是 ETH/USDT 永续合约实盘系统的 AI 持仓审核层。'
        '系统已经有仓位，且规则策略准备主动平仓或反手。'
        '你的选择只能是“close”或“hold”。'
        '如果你认为原策略该执行平仓/反手，则返回 close；如果认为应继续持有，则返回 hold。'
        '注意：硬止损、强平保护、动态回撤止盈不归你决定。'
        '必须只返回 JSON 对象，字段固定为 decision, confidence, reason, risk_flags。'
    )
    result = call_deepseek_json(system_prompt, context)
    decision = str(result.get('decision', '')).strip().lower()
    result['decision'] = decision
    result['approved'] = decision == 'close'
    return result


def guarded_entry_review(candidate_side, entry_reason, trigger_tf, curr_price, stop_loss_price, market_snapshot):
    """包一层容错策略：AI 出错时默认不新开仓。"""
    try:
        review = review_entry_with_ai(
            candidate_side=candidate_side,
            entry_reason=entry_reason,
        ...
        logging.info("开仓AI审核: %s", format_ai_review_text(review))
        return review
    except Exception as e:
        log_deepseek_config_warning_once(f"DeepSeek 开仓审核不可用: {e}")
        fallback = {
            'decision': 'allow' if DEEPSEEK_ALLOW_ENTRY_ON_ERROR else 'reject',
            'approved': DEEPSEEK_ALLOW_ENTRY_ON_ERROR,
            'confidence': 0.0,
            'reason': f'AI审核异常: {e}',
            'risk_flags': ['ai_error']
        }
        logging.warning("开仓AI审核失败，fallback=%s", format_ai_review_text(fallback))
        return fallback


def guarded_exit_review(position_side, proposed_action, trigger_label, curr_price, market_snapshot):
    """包一层容错策略：AI 出错时仍允许原策略做风险释放。"""
    try:
        review = review_exit_with_ai(
            position_side=position_side,
            proposed_action=proposed_action,
            trigger_label=trigger_label,
            curr_price=curr_price,
        ...
        logging.info("平仓AI审核: %s", format_ai_review_text(review))
        return review
    except Exception as e:
        log_deepseek_config_warning_once(f"DeepSeek 平仓审核不可用: {e}")
        fallback = {
            'decision': 'close' if DEEPSEEK_ALLOW_EXIT_ON_ERROR else 'hold',
            'approved': DEEPSEEK_ALLOW_EXIT_ON_ERROR,
            'confidence': 0.0,
            'reason': f'AI审核异常: {e}',
            'risk_flags': ['ai_error']
        }
        logging.warning("平仓AI审核失败，fallback=%s", format_ai_review_text(fallback))
        return fallback
...
def open_order(side, price, sl_price, state_4h, state_1h, state_15m, signal_bar_15m, entry_reason='trend', entry_trigger_tf='', ai_review=None):  # 定义执行开仓指令的函数，接收方向、当前价格、止损价格和各级别状态
    """执行开仓指令"""  # 函数的文档字符串
    ...
    try:  # 尝试执行开仓流程
        ...
        ai_review_text = format_ai_review_text(ai_review)

        def build_cond_str(state, side_dir):
            condensed = format_cond(state, side_dir)
            base = f"原因:{entry_reason} | {condensed}" if condensed else f"原因:{entry_reason}"
            return f"{base} | {ai_review_text}" if ai_review_text else base
        ...
        order_id_suffix = f"\n{order_id_lines}" if order_id_lines else ''
        ai_review_suffix = f"\nAI审核: {ai_review_text}" if ai_review_text else ''
...
def monitor_position(state_4h, state_1h, state_15m, atr_1h, atr_4h, signal_bar_15m='', allow_strategy_close=False, market_snapshot=None):  # 定义持仓监控函数，接收三个时间级别的状态评估结果和ATR值
    """持仓监控逻辑"""  # 函数的文档字符串
        ...
        if trade_state['side'] == 'long':  # 如果当前持仓方向为做多
            ...
            if allow_strategy_close:
                ...
                if entry_allowed and not reversal_conflict and reversal_short_state is not None and short_cond_15m and not lower_shadow_filter_short:
                    ai_review = guarded_exit_review(
                        position_side='long',
                        proposed_action='reverse_to_short',
                        trigger_label=f"{reversal_short_tf} 长上影确认反转平多并开空",
                        curr_price=curr_price,
                        market_snapshot=market_snapshot
                    )
                    if not ai_review.get('approved'):
                        logging.info(f"AI选择继续持有多单，忽略反手空信号: {format_ai_review_text(ai_review)}")
                        return
                    if close_position(f"{reversal_short_tf} 长上影确认反转平多并开空", curr_price, signal_bar_15m=signal_bar_15m, trigger_label=f"{reversal_short_tf} 长上影确认反转平多并开空", ai_review=ai_review):
                        try:
                ...
                if trend_short_ready and not trend_long_ready:
                    ai_review = guarded_exit_review(
                        position_side='long',
                        proposed_action='reverse_to_short',
                        trigger_label="趋势反手开空",
                        curr_price=curr_price,
                        market_snapshot=market_snapshot
                    )
                    if not ai_review.get('approved'):
                        logging.info(f"AI选择继续持有多单，忽略趋势反手空: {format_ai_review_text(ai_review)}")
                        return
                    if close_position("趋势空头条件成立，反手平多并开空", curr_price, signal_bar_15m=signal_bar_15m, trigger_label="趋势反手开空", ai_review=ai_review):
                        try:
                            reverse_price = get_latest_price()
                            reverse_sl = reverse_price + atr_4h
                ...
                if state_4h.get('close_long'):
                    ...
                    ai_review = guarded_exit_review(
                        ...
                        trigger_label="4H close_long",
                        curr_price=curr_price,
                        market_snapshot=market_snapshot
                    )
                    if not ai_review.get('approved'):
                        logging.info(f"AI选择继续持有多单，忽略4H close_long: {format_ai_review_text(ai_review)}")
                        return
                    close_position("4H close_long 策略平仓", curr_price, signal_bar_15m=signal_bar_15m, trigger_label="4H close_long", ai_review=ai_review)
                    return
                if state_1h.get('close_long'):
                    if REQUIRE_15M_CONFIRM_FOR_1H_CLOSE and not state_15m.get('close_long'):
                        ...
                        ai_review = guarded_exit_review(
                            position_side='long',
                            proposed_action='close_long',
                            trigger_label="1H close_long",
                            curr_price=curr_price,
                            market_snapshot=market_snapshot
                        )
                        if not ai_review.get('approved'):
                            logging.info(f"AI选择继续持有多单，忽略1H close_long: {format_ai_review_text(ai_review)}")
                            return
                        close_position("1H close_long 策略平仓", curr_price, signal_bar_15m=signal_bar_15m, trigger_label="1H close_long", ai_review=ai_review)
                        return
        ...
        # --- B. 空单监控 ---
            ...
            if allow_strategy_close:
                ...
                if entry_allowed and not reversal_conflict and reversal_long_state is not None and long_cond_15m and not upper_shadow_filter_long:
                    ai_review = guarded_exit_review(
                        position_side='short',
                        proposed_action='reverse_to_long',
                        trigger_label=f"{reversal_long_tf} 长下影确认反转平空并开多",
                        curr_price=curr_price,
                        market_snapshot=market_snapshot
                    )
                    if not ai_review.get('approved'):
                        logging.info(f"AI选择继续持有空单，忽略反手多信号: {format_ai_review_text(ai_review)}")
                        return
                    if close_position(f"{reversal_long_tf} 长下影确认反转平空并开多", curr_price, signal_bar_15m=signal_bar_15m, trigger_label=f"{reversal_long_tf} 长下影确认反转平空并开多", ai_review=ai_review):
                        try:
                            reverse_price = get_latest_price()
                ...
                if trend_long_ready and not trend_short_ready:
                    ai_review = guarded_exit_review(
                        position_side='short',
                        proposed_action='reverse_to_long',
                        trigger_label="趋势反手开多",
                        curr_price=curr_price,
                        market_snapshot=market_snapshot
                    )
                    if not ai_review.get('approved'):
                        logging.info(f"AI选择继续持有空单，忽略趋势反手多: {format_ai_review_text(ai_review)}")
                        return
                    if close_position("趋势多头条件成立，反手平空并开多", curr_price, signal_bar_15m=signal_bar_15m, trigger_label="趋势反手开多", ai_review=ai_review):
                        try:
                            reverse_price = get_latest_price()
                            reverse_sl = reverse_price - atr_4h
                ...
                if state_4h.get('close_short'):
                    ...
                    ai_review = guarded_exit_review(
                        position_side='short',
                        proposed_action='close_short',
                        trigger_label="4H close_short",
                        curr_price=curr_price,
                        market_snapshot=market_snapshot
                    )
                    if not ai_review.get('approved'):
                        logging.info(f"AI选择继续持有空单，忽略4H close_short: {format_ai_review_text(ai_review)}")
                        return
                    close_position("4H close_short 策略平仓", curr_price, signal_bar_15m=signal_bar_15m, trigger_label="4H close_short", ai_review=ai_review)
                    return
                if state_1h.get('close_short'):
                    if REQUIRE_15M_CONFIRM_FOR_1H_CLOSE and not state_15m.get('close_short'):
                        ...
                        ai_review = guarded_exit_review(
                            position_side='short',
                            proposed_action='close_short',
                            trigger_label="1H close_short",
                            curr_price=curr_price,
                            market_snapshot=market_snapshot
                        )
                        if not ai_review.get('approved'):
                            logging.info(f"AI选择继续持有空单，忽略1H close_short: {format_ai_review_text(ai_review)}")
                            return
                        close_position("1H close_short 策略平仓", curr_price, signal_bar_15m=signal_bar_15m, trigger_label="1H close_short", ai_review=ai_review)
                        return
...
def close_position(reason, curr_price=None, signal_bar_15m='', trigger_label='', ai_review=None):  # 定义平仓函数，接收平仓原因和当前触发价格
    """执行平仓指令"""  # 函数的文档字符串
...
def run_strategy():  # 定义策略运行的主函数，负责统筹数据获取、状态判断和下单
    ...
    market_snapshot = build_market_snapshot_for_ai(
        df_4h, df_1h, df_15m,
        state_4h, state_1h, state_15m,
        signal_bar_15m,
    ...
    if trade_state['has_position']:  # 判断当前是否已经有仓位在手
        monitor_position(state_4h, state_1h, state_15m, atr_1h, atr_4h, signal_bar_15m=signal_bar_15m, allow_strategy_close=is_new_signal_bar, market_snapshot=market_snapshot)  # 如果有持仓，进入监控持仓和止盈止损逻辑
    ...
    if not reversal_conflict and reversal_long_state is not None and long_cond_15m and not upper_shadow_filter_long:
        try:
            ...
            ai_review = guarded_entry_review(
                candidate_side='long',
                entry_reason='shadow_reversal_long',
                trigger_tf=reversal_long_tf,
                curr_price=curr_price,
                stop_loss_price=sl_price,
                market_snapshot=market_snapshot
            )
            if not ai_review.get('approved'):
                logging.info(f"AI拒绝影线反转多单: {format_ai_review_text(ai_review)}")
                trade_state['last_processed_bar_15m'] = signal_bar_15m
                return
            # entry_reason 会被写进状态、通知和 CSV，后面一看就知道是影线反转多单
            open_order('long', curr_price, sl_price, state_4h, state_1h, state_15m, signal_bar_15m, entry_reason='shadow_reversal_long', entry_trigger_tf=reversal_long_tf, ai_review=ai_review)
    ...
    if not reversal_conflict and reversal_short_state is not None and short_cond_15m and not lower_shadow_filter_short:
        try:
            ...
            ai_review = guarded_entry_review(
                ...
                entry_reason='shadow_reversal_short',
                trigger_tf=reversal_short_tf,
                curr_price=curr_price,
                stop_loss_price=sl_price,
                market_snapshot=market_snapshot
            )
            if not ai_review.get('approved'):
                logging.info(f"AI拒绝影线反转空单: {format_ai_review_text(ai_review)}")
                trade_state['last_processed_bar_15m'] = signal_bar_15m
                return
            # entry_reason 会被写进状态、通知和 CSV，后面一看就知道是影线反转空单
            open_order('short', curr_price, sl_price, state_4h, state_1h, state_15m, signal_bar_15m, entry_reason='shadow_reversal_short', entry_trigger_tf=reversal_short_tf, ai_review=ai_review)
    ...
    if trend_long_ready:  # 如果三个级别的做多条件同时满足（共振）
        try:
            ...
            ai_review = guarded_entry_review(
                ...
                trigger_tf='4H+1H+15M',
                curr_price=curr_price,
                stop_loss_price=sl_price,
                market_snapshot=market_snapshot
            )
            if not ai_review.get('approved'):
                logging.info(f"AI拒绝趋势多单: {format_ai_review_text(ai_review)}")
                trade_state['last_processed_bar_15m'] = signal_bar_15m
                return
            open_order('long', curr_price, sl_price, state_4h, state_1h, state_15m, signal_bar_15m, entry_reason='trend_long', entry_trigger_tf='4H+1H+15M', ai_review=ai_review)  # 调用开仓函数，执行做多操作
            logging.info("多单开仓成功")
        except Exception:
    ...
    if trend_short_ready:  # 如果三个级别的做空条件同时满足（共振）
        try:
            ...
            if not ai_review.get('approved'):
                logging.info(f"AI拒绝趋势空单: {format_ai_review_text(ai_review)}")
                trade_state['last_processed_bar_15m'] = signal_bar_15m
                return
            open_order('short', curr_price, sl_price, state_4h, state_1h, state_15m, signal_bar_15m, entry_reason='trend_short', entry_trigger_tf='4H+1H+15M', ai_review=ai_review)  # 调用开仓函数，执行做空操作
            logging.info("空单开仓成功")
        except Exception:
...
if __name__ == '__main__':  # Python标准写法，判断当前文件是否作为主程序被直接运行
    try:
        ...
        logging.info(
            "DeepSeek AI 审核层: enabled=%s, model=%s, thinking=%s, base_url=%s",
            DEEPSEEK_AI_ENABLED,
            DEEPSEEK_MODEL,
        ...
        if DEEPSEEK_AI_ENABLED and not DEEPSEEK_API_KEY:
            log_deepseek_config_warning_once(
                "DeepSeek AI 已启用但缺少 DEEPSEEK_API_KEY；新开仓会被 AI 审核层拒绝，策略性平仓仍按原逻辑执行。"
            )
    except Exception as e:
