...
def refresh_protective_stop_order(stop_price):
    ...
    try:
        ...
        trade_state['stop_order_id'] = stop_order.get('id', '')
...
def log_trade_to_csv(entry_time, side, cond_4h, cond_1h, cond_15m, entry_reason, exit_time, close_reason, pnl_points, fee_cost, net_pnl_usdt, is_profit, entry_signal_bar_15m='', exit_signal_bar_15m='', exit_trigger='', holding_seconds=0):
    """将交易记录写入CSV文件，按月分表"""
    ...
    try:
        with open(filename, mode='a', newline='', encoding='utf-8-sig') as f:
            ...
            if not file_exists:
                ...
                writer.writerow(['建仓时间', '趋势方向', '4H条件', '1H条件', '15M条件', '入场原因', '平仓时间', '平仓原因', '点数盈亏', '手续费', '净利润(USDT)', '是否盈利', '入场15M信号时间', '平仓15M信号时间', '平仓触发周期', '持仓秒数'])
            # 写入具体数据
            writer.writerow([entry_time, side, cond_4h, cond_1h, cond_15m, entry_reason, exit_time, close_reason, pnl_points, fee_cost, net_pnl_usdt, is_profit, entry_signal_bar_15m, exit_signal_bar_15m, exit_trigger, holding_seconds])
    except Exception as e:
...
def open_order(side, price, sl_price, state_4h, state_1h, state_15m, signal_bar_15m, entry_reason='trend'):  # 定义执行开仓指令的函数，接收方向、当前价格、止损价格和各级别状态
    ...
    try:  # 尝试执行开仓流程
        ...
        if not stop_price_is_still_valid(actual_price, actual_safe_stop, side):
            ...
            exchange.create_market_order(SYMBOL, panic_side, amount)
            send_msg(
                "ETH交易: ⚠️开仓后立即撤退",
        ...
        msg = (f"🚀 【已开仓】\n方向: {side}\n入场价: {actual_price}\n"  # 构建通知邮件的内容字符串，包含方向和入场价
               ...
               f"杠杆: {LEVERAGE}x\n入场原因: {entry_reason}\n15M信号时间: {signal_bar_15m}")  # 继续构建字符串，包含止损价、数量和杠杆倍数
        send_msg(f"ETH交易: 开仓 {side}", msg)  # 调用发邮件函数，发送开仓通知
        logging.info(
            f"开仓成功: {side} at {actual_price}, SL: {actual_safe_stop}, "
            f"liq={actual_liquidation_price}, reason={entry_reason}, stop_meta={actual_stop_meta}"
        )  # 在系统日志中记录开仓成功的详细信息
        return True
                        ...
                        if protect_ref is not None and not pd.isna(protect_ref):
                            ...
                            if new_sl > trade_state['stop_loss_price']:
                                ...
                                if not refresh_protective_stop_order(new_sl):
                                    ...
                                    return
                                send_msg("ETH交易: 多单收紧止损", f"{tighten_tf} 长上影信号触发，新的多单止损价: {new_sl:.4f}")
                        ...
                        if protect_ref is not None and not pd.isna(protect_ref):
                            ...
                            if new_sl < trade_state['stop_loss_price']:
                                ...
                                if not refresh_protective_stop_order(new_sl):
                                    close_position("服务端止损更新失败，主动平仓", curr_price, signal_bar_15m=signal_bar_15m, trigger_label="服务端止损更新失败")
                                    return
                                send_msg("ETH交易: 空单收紧止损", f"{tighten_tf} 长下影信号触发，新的空单止损价: {new_sl:.4f}")
...
def close_position(reason, curr_price=None, signal_bar_15m='', trigger_label=''):  # 定义平仓函数，接收平仓原因和当前触发价格
    ...
    try:  
        ...
        logging.info(f"清仓成功: {reason}, PnL: {pnl_points:.2f}, Net USDT: {net_pnl_usdt:.2f}")  # 记录清仓成功的日志，包含原因和点数盈亏
        return True
...
def run_strategy():  # 定义策略运行的主函数，负责统筹数据获取、状态判断和下单
    ...
    # df_4h.to_csv("df_4h.csv",mode="w",index=False)
    # df_1h.to_csv("df_1h.csv",mode="w",index=False)
    # df_15m.to_csv("df_15m.csv",mode="w",index=False)
    # 2. 评估各个级别的状态
