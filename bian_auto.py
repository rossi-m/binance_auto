...
HEARTBEAT_INTERVAL_SECONDS = 60  # 每60秒输出一次心跳日志，方便判断进程是否还活着
...
def reset_trade_state_after_external_close(signal_bar_15m='', reason='检测到交易所仓位已关闭', external_context=None):
    ...
    if estimated_close_price is None:
        try:
            ...
            close_price_note = '按检测时最新价估算'
        except Exception as e:
            logging.warning(f"外部平仓估算平仓价失败，将按0点数记录: {e}")
    else:
        close_price_note = '按服务端止损价估算'
    ...
    if not balance_pnl_available:
        net_pnl_usdt = pnl_points * amount - fee_cost if amount > 0 else 0.0
        is_profit = net_pnl_usdt > 0
    ...
    close_price_text = f"{estimated_close_price}" if estimated_close_price is not None else "未获取"
    fee_text = f"{fee_cost:.4f}"
    net_pnl_text = f"{net_pnl_usdt:.2f}"
    final_usdt_text = f"{final_usdt:.4f}" if final_usdt is not None else "未获取"
    pnl_points_text = f"{pnl_points:.2f}"
    net_pnl_note = '' if balance_pnl_available else '（按估算出场价推算）'
    send_msg(
        "ETH交易: ⚠️检测到外部平仓",
        f"原因: {reason}\n"
        ...
        f"出场价: {close_price_text}（{close_price_note}）\n"
        f"点数盈亏: {pnl_points_text}\n"
        f"手续费: {fee_text}\n"
        f"净利润(USDT): {net_pnl_text}{net_pnl_note}\n"
...
def evaluate_trend(df, timeframe, time_factor, is_4h=False, now_dt=None):  # 定义评估趋势状态的函数，接收数据、时间系数和是否为4H周期的标识
    ...
    if is_4h:  # 如果当前评估的是4小时级别数据，执行以下特定逻辑
        ...
        lt_score_rsi_val = last['rsi']-prev['rsi']>3
        lt_score = sum([
            lt_score_rsi_val,
        ...
        st_score_rsi_val = last['rsi']-prev['rsi']<-3
        st_score_volume_expand = last['volume'] > prev['volume']
        st_score_rsi_zone = 32 <= last['rsi'] <= 70
        st_score_macd_momentum = (last['macd'] < last['macd_signal']) and (abs(last['macd_hist']) >= asb(prev['macd_hist']))
        ...
        ps_c2 = last['rsi'] >= 35 and last['rsi']-prev['rsi']<-3  # 条件2：当前RSI大于35
        ...
        lt_c2 = 35<=last['rsi'] <= 65 and last['rsi']-prev['rsi']>3 # 条件2：当前RSI小于65
        ...
        pl_c2 = 35 <= last['rsi'] <= 65 and last['rsi']-prev['rsi']>3  # 条件2：当前RSI的值在40到65之间
        ...
        st_c2 = 35 <= last['rsi'] <= 65 and last['rsi']-prev['rsi']>-3  # 条件2：当前RSI的值在40到60之间
    ...
    if timeframe in ('4h', '1h'):
        ...
        # 开空反转：最近几根里先出现压力区长上影，后续任意一根只要跌到该K实体中点，且当前未回到多头逻辑，就允许触发
        ...
        # 开多反转：最近几根里先出现支撑区长下影，后续任意一根只要涨到该K实体中点，且当前未回到空头逻辑，就允许触发
        ...
        if trade_state['side'] == 'long':  # 如果当前持仓方向为做多
            ...
            if dynamic_retrace:
                ...
                if should_refresh_dynamic_stop('long', trade_state['stop_loss_price'], dynamic_stop, atr_1h, entry):
                    ...
                    if not refresh_protective_stop_order(dynamic_stop):
                        close_position(
                            "动态回撤保护止损更新失败，主动平仓",
                            curr_price,
                            signal_bar_15m=signal_bar_15m,
                            trigger_label="动态回撤保护止损更新失败"
                        )
                        return
            ...
            if allow_strategy_close:
                ...
                if reversal_short_state is not None:
                    close_position(f"{reversal_short_tf} 长上影确认反转平多", curr_price, signal_bar_15m=signal_bar_15m, trigger_label=f"{reversal_short_tf} 长上影确认反转平多")
                    return
        ...
        # --- B. 空单监控 ---
            ...
            if allow_strategy_close:
                ...
                if reversal_long_state is not None:
                    close_position(f"{reversal_long_tf} 长下影确认反转平空", curr_price, signal_bar_15m=signal_bar_15m, trigger_label=f"{reversal_long_tf} 长下影确认反转平空")
                    return
