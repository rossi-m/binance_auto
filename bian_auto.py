...
def evaluate_trend(df, timeframe, time_factor, is_4h=False, now_dt=None):  # 定义评估趋势状态的函数，接收数据、时间系数和是否为4H周期的标识
    ...
    if timeframe in ('4h', '1h'):
        ...
        upper_shadow_mid_hit = upper_shadow_candidate is not None and last['low'] <= upper_shadow_candidate['body_mid']
        lower_shadow_mid_hit = lower_shadow_candidate is not None and last['high'] >= lower_shadow_candidate['body_mid']
