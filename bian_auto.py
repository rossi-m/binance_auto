...
RETRACE_THRESHOLD = 0.4  # 设置利润回撤阈值：利润从最高点回调40%时触发清仓
...
def format_condition_snapshot_for_mail(timeframe, state):
    ...
    parts = [
        ...
        f"pullback_long={state.get('pullback_long')}",
        f"short_trend={state.get('short_trend')}",
        f"pullback_short={state.get('pullback_short')}",
        f"close_long={state.get('close_long')}",
        f"close_short={state.get('close_short')}",
        f"upper_shadow_filter_long={state.get('upper_shadow_filter_long')}",
        f"lower_shadow_filter_short={state.get('lower_shadow_filter_short')}",
        f"upper_shadow_reversal_short={state.get('upper_shadow_reversal_short')}",
        f"lower_shadow_reversal_long={state.get('lower_shadow_reversal_long')}",
        f"upper_shadow_tighten_long={state.get('upper_shadow_tighten_long')}",
        f"lower_shadow_tighten_short={state.get('lower_shadow_tighten_short')}",
        f"lt={details.get('lt')}",
        f"pl={details.get('pl')}",
        f"st={details.get('st')}",
        f"ps={details.get('ps')}",
...
def evaluate_trend(df, timeframe, time_factor, is_4h=False, now_dt=None):  # 定义评估趋势状态的函数，接收数据、时间系数和是否为4H周期的标识
    ...
    if is_4h:  # 如果当前评估的是4小时级别数据，执行以下特定逻辑
        ...
        ps_c4 = last['rsi'] > 40  # 条件4：最新收盘蜡烛图 RSI14 > 40
        res['pullback_short'] = sum([ps_c_ema_boll, ps_c3, ps_c4]) >= 3  # 3个条件满足至少2个，则判定为回调空头结构
        ...
        lt_gate = sum([lt_gate_ema, lt_gate_boll, lt_gate_candle]) >= 2

        lt_score_structure_ok = lt_gate_ema and lt_gate_boll
        lt_score_volume_expand = last['volume'] > prev['volume']
        lt_score_rsi_zone = 40 <= last['rsi'] <= 68
        lt_score_macd_momentum = (last['macd'] > last['macd_signal']) and (last['macd_hist'] >= prev['macd_hist'])
        lt_score_price_progress = last['close'] > prev['close']
        lt_score = sum([
            lt_score_structure_ok,
        ...
        pl_c4 = last['rsi'] < 70  # 条件4：最新收盘蜡烛图 RSI14 < 70
        res['pullback_long'] = sum([pl_c_ema_boll, pl_c3, pl_c4]) >= 3  # 满足至少2个条件，即判定为回调多头结构
        ...
        st_gate = sum([st_gate_ema, st_gate_boll, st_gate_candle]) >= 2

        st_score_structure_ok = st_gate_ema and st_gate_boll
        st_score_volume_expand = last['volume'] > prev['volume']
        st_score_rsi_zone = 32 <= last['rsi'] <= 70
        st_score_macd_momentum = (last['macd'] < last['macd_signal']) and (last['macd_hist'] <= prev['macd_hist'])
        st_score_price_progress = last['close'] < prev['close']
        st_score = sum([
            st_score_structure_ok,
        ...
        # 记录 4H 级别的具体条件判断结果
        res['details'] = {
            'ps': {'ema_or_boll': ps_c_ema_boll, 'c1_ema': ps_c1, 'c2_boll': ps_c2, 'c3_vol': ps_c3, 'c4_rsi': ps_c4},
            'lt': {
                'gate': lt_gate,
                'score': lt_score,
                ...
                'structure_ok': lt_score_structure_ok,
            ...
            'pl': {'ema_or_boll': pl_c_ema_boll, 'c1_ema': pl_c1, 'c2_boll': pl_c2, 'c3_vol': pl_c3, 'c4_rsi': pl_c4},
            'st': {
                'gate': st_gate,
                'score': st_score,
                ...
                'structure_ok': st_score_structure_ok,
        ...
        ps_c2 = last['rsi'] >= 35  # 条件2：当前RSI大于35
        ...
        lt_c2 = 35<=last['rsi'] <= 65  # 条件2：当前RSI小于65
        ...
        pl_c2 = 35 <= last['rsi'] <= 65  # 条件2：当前RSI的值在40到65之间
        ...
        st_c2 = 35 <= last['rsi'] <= 65  # 条件2：当前RSI的值在40到60之间
...
def open_order(side, price, sl_price, state_4h, state_1h, state_15m, signal_bar_15m, entry_reason='trend'):  # 定义执行开仓指令的函数，接收方向、当前价格、止损价格和各级别状态
    ...
    try:  # 尝试执行开仓流程
        ...
        open_condition_details = (
            f"4H({state_4h.get('signal_bar_time', '')}): {cond_4h_str}\n"
            f"1H({state_1h.get('signal_bar_time', '')}): {cond_1h_str}\n"
            f"15M({state_15m.get('signal_bar_time', '')}): {cond_15m_str}"
        )
