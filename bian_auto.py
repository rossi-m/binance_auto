...
def evaluate_trend(df, timeframe, time_factor, is_4h=False, now_dt=None):  # 定义评估趋势状态的函数，接收数据、时间系数和是否为4H周期的标识
    ...
    if is_4h:  # 如果当前评估的是4小时级别数据，执行以下特定逻辑
        ...
        res['pullback_short'] = sum([ps_c1, ps_c2, ps_c3, ps_c4]) >= 3  # 上述4个条件满足至少3个，则判定为回调空头结构
        ...
        lt_c4 = last['rsi'] < 70  # 条件4：最新收盘蜡烛图 RSI14 < 70
        res['long_trend'] = sum([lt_c1, lt_c2, lt_c3, lt_c4]) >= 3  # 上述4个条件中满足至少3个，则判定为多头趋势
        
        # B. 多头转空头 -> 回调趋势还是多头 (满足任意3个，有容错)
        ...
        res['pullback_long'] = sum([pl_c1, pl_c2, pl_c3, pl_c4]) >= 3  # 满足至少3个条件，即判定为回调多头结构

        # B. 多头转空头 -> 空头趋势 (满足3个点，有容错)
